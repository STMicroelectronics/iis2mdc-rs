...
Run the *read_polling* example on the Nucleo F401RE using the stm32-rs framework:
```sh
cargo nucleo-f401re --features read_polling
```

Run the same example using the Embassy async framework:
```sh
cargo nucleo-f401re-embassy --features read_polling
```
...
define_embassy_with_st_link! {
    i2c = {
        address: I2CAddress::I2cAdd as u8,
        periph: I2C1,
        scl: PB8,
        sda: PB9,
        ev_irq: I2C1_EV,
